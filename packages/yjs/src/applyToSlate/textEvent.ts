...
function applyDelta(node: Element, slatePath: Path, delta: Delta): Operation[] {
  ...
  delta.reverse().forEach(change => {
    ...
    if ('insert' in change) {
      let [pathOffset, textOffset] = yOffsetToSlateOffsets(node, yOffset, {
        insert: true,
      })
      let child = node.children[pathOffset]

      if (
        typeof change.insert === 'string' &&
        pathOffset > 0 &&
        (child == null || textOffset === 0)
      ) {
        const previousPathOffset = pathOffset - 1
        const previousChild = node.children[previousPathOffset]

        if (
          Text.isText(previousChild) &&
          deepEquals(change.attributes ?? {}, getProperties(previousChild))
        ) {
          pathOffset = previousPathOffset
          textOffset = previousChild.text.length
          child = previousChild
        }
      }
      ...
      if (Text.isText(child)) {
        ...
        if (
          ...
          (lastOp == null || Path.equals(childPath, lastPath))
        ) {
          return ops.push({
            type: 'insert_text',
            offset: textOffset,
